...
- [x] 로컬 파일 시스템 캐시 드라이버
- [x] S3 캐시 드라이버 (버킷 설정 가능)
- [x] TTL/만료 정책: LRU, max size 기반 정리 기능

---

## 🔐 인증/인가/접근 제어

- [x] IP 화이트리스트 + CIDR 기반 필터
- [x] BasicAuth (설정파일 기반)
- [x] 사용자별 권한 정책 (읽기/쓰기/삭제 허용)
- [x] 허용된 패키지 목록 기반 필터 (AllowList)

---

## 🛡️ 보안 및 검증

- [x] SHA256 해시 검증 (패키지 메타와 비교)
- [x] 요청 응답 기록 로깅 (`access.log`)
- [x] 패키지 검증 실패 시 차단/알림
...
- [x] 통합 테스트: 실제 패키지 요청을 모사하는 시나리오 테스트
- [x] 단위 테스트: 라우팅, 캐시 hit, 인증 등
- [x] 로컬 e2e 테스트용 Docker Compose 환경 제공
...
- [x] `/healthz` 상태 체크 API
- [x] structured logging (JSON + leveled log)

---

## 🐳 실행 및 배포

- [x] 다중 아키텍처 Docker 이미지 빌드 (`amd64`, `arm64`)
- [x] CI 파이프라인에서 자동 테스트 & 빌드
- [x] Helm chart or systemd 서비스 템플릿 제공
...
- [x] `conf/config.example.yaml`
- [x] REF.md: APT/PIP/NPM 패키지 흐름 및 헤더 구조 설명
- [x] 패키지별 설정 예시 (APT sources.list 등)
