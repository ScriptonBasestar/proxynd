...
- 전체 또는 특정 패키지 타입별 정리
- 확인 프롬프트 제공
- **시간 기반 정리**: `--older-than` 플래그로 지정된 시간보다 오래된 캐시만 정리
  - 예: `--older-than 7d` (7일), `--older-than 2h` (2시간), `--older-than 30m` (30분)
- **크기 기반 정리**: `--size-limit` 플래그로 캐시 총 크기 제한
  - 예: `--size-limit 1GB`, `--size-limit 500MB`
...
- 권한 설정 (읽기/쓰기/관리자)
- 패스워드 생성 또는 입력
- **대화형 모드 (초과 구현)**: `--interactive` 플래그로 안전한 비밀번호 입력 지원
  - 비밀번호 입력 시 화면에 표시되지 않음 (보안 강화)
  - 비밀번호 확인 프롬프트 제공
  - 역할 선택 가이드 제공 (user/admin)
  - 기존 옵션과 조합 가능 (`-i -r admin`, `-i -u newuser`)
- **사용자 설명**: `--description` 플래그로 사용자 설명 추가
- **역할 기반 권한**: `--role` 플래그로 세밀한 권한 제어 (user/admin)
...
#### `proxyndctl user info <username>`
- **개별 사용자 상세 정보 조회** (초과 구현)
- **제공 정보**:
...
ProxyND는 Maven 생태계에 대한 추가적인 관리 기능을 제공합니다.

### `proxyndctl maven-index`
- **Maven 인덱스 관리** (초과 구현)
- 로컬 Maven 인덱스 업데이트
- 인덱스 무결성 검증
- 인덱스 통계 및 최적화

### `proxyndctl maven-backup`
- **Maven 백업 관리** (초과 구현)  
- 중요한 Maven 아티팩트 백업
- 백업 스케줄링 및 관리
- 복원 기능 지원

이러한 확장 기능들은 Maven 중앙 저장소를 대량으로 사용하는 기업 환경에서 특히 유용합니다.
