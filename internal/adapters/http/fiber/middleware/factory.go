...
func (f *MiddlewareFactory) BuildMiddlewareChain() ports.MiddlewareChain {
	chain := &MiddlewareChain{
		middlewares: make([]ports.EnhancedHTTPMiddleware, 0),
	}
...
// SetHeader sets response header value
...
// SendString sends string response (without status)
func (f *FiberHTTPContext) SendString(s string) error {
	return f.ctx.SendString(s)
}

// SendStringWithStatus sends string response with status
func (f *FiberHTTPContext) SendStringWithStatus(status int, s string) error {
