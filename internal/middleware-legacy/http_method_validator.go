...
func ReadOnlyMethodValidator() fiber.Handler {
	return func(c *fiber.Ctx) error {
		method := c.Method()
		log.Printf("ReadOnlyMethodValidator: Checking method %s for path %s", method, c.Path())

		// Allow only GET and HEAD methods for read-only proxy
		if method == fiber.MethodGet || method == fiber.MethodHead {
			log.Printf("ReadOnlyMethodValidator: Allowed method %s", method)
			return c.Next()
		}

		// Reject all other methods with 405 Method Not Allowed
		log.Printf("ReadOnlyMethodValidator: Rejecting method %s with 405", method)
