...
func TestMavenHandlerV2_Simple(t *testing.T) {
	...
	// NOTE: isSnapshotArtifact method does not exist - test commented out
	/*
	...
	*/
	...
	t.Run("isChecksumFile", func(t *testing.T) {
		...
		// 체크섬 파일들
		assert.True(t, isChecksumFile(testMavenChecksumPath))
		assert.True(t, isChecksumFile("com/example/app/1.0/app-1.0.jar.md5"))
		assert.True(t, isChecksumFile("com/example/app/1.0/app-1.0.jar.sha256"))
		assert.True(t, isChecksumFile("com/example/app/1.0/app-1.0.jar.sha512"))

		// 일반 파일
		assert.False(t, isChecksumFile("com/example/app/1.0/app-1.0.jar"))
		assert.False(t, isChecksumFile("com/example/app/1.0/app-1.0.pom"))
	})

	// NOTE: validateArtifactPath method does not exist - test commented out
	/*
